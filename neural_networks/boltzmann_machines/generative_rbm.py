...
class GenRBM:
    ...
    def __init__(self, num_visible_units, num_hidden_units, weights=None):
        ...
        self.W = np.random.randn(
            num_visible_units + 1, num_hidden_units + 1) * 0.1
        ...
        print(
            'Initialized Generative RBM with {0} visible' +
            ' and {1} hidden units.'.format(
                num_visible_units, num_hidden_units))

    def train(
            self, trainX, epochs=10, learning_rate=0.05,
        ...
        for epoch in range(epochs):
            ...
            # penalize weights
            L1 = np.sum(np.absolute(self.W)) * lambda_1
            L2 = np.sum(self.W**2) * lambda_2
            self.W -= learning_rate * (L1 + L2)
            ...
            # log progress
            print log_str + ' reconstruction error={0}'.format(
                error / len(trainX))

        print 'Best reconstruction error={0} in epoch {1}.'.format(
            self.e_best, self.Epoch_best)

    def sample_hidden(self, V, use_softmax=False, use_best=False):
        if V.shape[1] == self.W.shape[0] - 1:
            V = np.insert(V, 0, 1, axis=1)
        if use_softmax:
            return f_softmax(np.dot(
                V, self.W_best if use_best else self.W))[:, 1:]
        return f_sigmoidal(np.dot(
            V, self.W_best if use_best else self.W))[:, 1:]
    ...
    def sample_visible(self, H, use_softmax=False, use_best=False):
        ...
        if use_softmax:
            return f_softmax(
                np.dot(H, self.W_best.T if use_best else self.W.T))[:, 1:]
        return f_sigmoidal(
            np.dot(H, self.W_best.T if use_best else self.W.T))[:, 1:]
    ...
    def dream(self, epochs, x=None, probabilities=True, use_best=False):
        ...
        for e in range(epochs):
            ...
            P_v = f_sigmoidal((P_h > np.random.rand(*P_h.shape)).dot(
                (self.W_best if use_best else self.W).T))
            P_v[:, 0] = 1
            X[e + 1, :] = P_v[e, :] \
                if probabilities else P_v[e, :] > np.random.rand(P_v.shape[1])
        return X[:, 1:]
